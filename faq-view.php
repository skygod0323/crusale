...
function toggleFAQ( argID ){
    $('#'+argID+'_cnt').toggle();
	$('#'+argID).toggleClass('current');
}    
</script>

<div><h2 class="header-txt1-style align-center-txt"><?php echo $lang[310]; ?></h2></div>
...
<div class="row">
	...
	<!-- PAGE CONTENT BEGINS -->
		...
		<div class="tabbable">
            ...
            <?php 
      			$sql_fcat_h="select * from faq_categories order by id";
				$res_fcat_h=mysql_query($sql_fcat_h);
				$i=0;
				while($row_fcat_h=mysql_fetch_object($res_fcat_h)){
			?>
				<li <?php if($i==0){	?>class="active"	<?php	}	?>	>
					<a data-toggle="tab" href="#faq-tab-<?php echo $row_fcat_h->id; ?>">
						<i class="blue icon-question-sign bigger-120"></i>
						<?php echo $row_fcat_h->category; ?>
					</a>
				</li>
            <?php	
			$i++;
        ...
        <?php 
			$sql_fcat="select * from faq_categories";
			$res_fcat=mysql_query($sql_fcat);
			$j=0;
			while($row_fcat=mysql_fetch_object($res_fcat)){
		?>
			<div id="faq-tab-<?php echo $row_fcat->id; ?>" class="tab-pane fade <?php if($j==0){	?> in active<?php } ?>">
				<h4 class="blue">
					<i class="icon-ok bigger-110"></i>
					<?php echo $row_fcat->category; ?>
				</h4>
				<div class="space-8"></div>
				<div id="faq-list-<?php echo $row_fcat->id; ?>" class="panel-group accordion-style1 accordion-style2">
                <?php
					$sql_faq="select * from custom_faq where cat_id='".$row_fcat->id."'";
					$res_faq=mysql_query($sql_faq);
					while($row_faq=mysql_fetch_object($res_faq)){
				?>
					<div class="panel panel-default">
						<div class="panel-heading" align="left">
							<a href="#faq-<?php echo $row_fcat->id; ?>-<?php echo $row_faq->id; ?>" data-parent="#faq-list-<?php echo $row_fcat->id; ?>" data-toggle="collapse" class="accordion-toggle collapsed">
								...
								<i class="icon-copy bigger-130"></i>
									&nbsp; <?php echo $row_faq->heading; ?>
							</a>
						</div>
						<div class="panel-collapse collapse" id="faq-<?php echo $row_fcat->id; ?>-<?php echo $row_faq->id; ?>">
							<div class="panel-body" align="left">
								<?php echo stripcslashes($row_faq->content); ?>
							</div>
						</div>
					</div>
				<?php	}	?>
