...
$_SESSION['last_page']="bidplaced.php";

$prj_id=$_SESSION['lst_prj'];


if(isset($_SESSION['msg'])){	$msg=$_SESSION['msg'];	unset($_SESSION['msg']);	}

if(isset($_SESSION['bo']) && isset($_SESSION['tot_bo_amt']))
{
	...
	if($row_chb->usr_balance < $_SESSION['tot_bo_amt'])
	{
          $_SESSION['promotion_msg']=$lang[691];
		header("location:payment-deposit.php");
	}
	else
	{
		foreach($_SESSION['bo'] as $val)
		{
			/*$sql_bo="insert into bidoption
				set
					b_bd_id='".$_SESSION['lst_bd']."',
					b_bo_id='".$val."'";	
					
			mysql_query($sql_bo);*/
                  if($val=='1')
                  {
				$sql_upd="update bid set bd_uplift='1' where bd_id='".$_SESSION['lst_bd']."'";
				mysql_query($sql_upd);
			}
			else if($val=='2')
			{
				$sql_upd="update bid set bd_highlight='1' where bd_id='".$_SESSION['lst_bd']."'";
				mysql_query($sql_upd);
			}
		}
		...
		/*$sql_empl="select * from user where usr_id=(select prj_usr_id from project where prj_id='".$prj_id."')";
...
<div><?php echo $lang[271]; ?><b><?php echo $rowp->prj_name; ?></b><?php echo $lang[272]; ?></div>
		

		
        <a class="btn btn-info" href="project.php?p=<?php echo $prj_id; ?>">
            <span><?php echo $lang[273]; ?></span>
        </a>
