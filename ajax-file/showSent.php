	...
	<div class="my-message-header-bg-arrow-str">
		<ul>
                    ...
                    if ($previous_btn && $cur_page > 1){
                        $pre = $cur_page - 1;
                    ?>
                      <li><a href="javascript:showSent('<?php echo $pre; ?>')"><img src="images/lft-arrow.jpg" alt="" /></a></li>
                    ...
                    if($next_btn && $cur_page < $no_of_paginations)
                    {
                        $nex = $cur_page + 1;
                    ?>
                      <li><a href="javascript:showSent('<?php echo $nex; ?>')"><img src="images/rgt-arrow.jpg" alt="" /></a></li>
                    <?php
                    }else if ($next_btn){
                    ?>
		...
		<div class="my-message-body">
			<ul>
			 ...
			 while($row=mysql_fetch_object($recObj)){ ?>
				<li class="clearfix" onClick="showSentDetail(<?php echo $row->msg_id; ?>)">
				...
				<?php } ?>
					...
					<h6><?php echo $row->usr_name; ?></h6>
					<p class="date">
                              <?php 
						$diff=dateDifference($row->msg_date,date("Y-m-d h:i:s"));
						if($diff>0){	echo $diff.$lang[74];	}else {	echo $lang[354];	}
					?>
                              </p>
					</div>
					</div>
					<div class="message-body-rgt-col">
						<div class="my-message-body-check-bx-txt">&nbsp;&nbsp;<?php echo $row->prj_name; ?>
                                    <div style="width: 15px;float:right;padding-right: 2px;"><a style="cursor: pointer;" onClick="delMessage(<?php echo $row->msg_id; ?>,'from')"><img src="images/msg_delete.png"/></a></div>
                                    </div>
						<div class="my-message-body-welcome-txt"><?php echo $row->msg_message;  ?></div>
					</div>
                            <li class="clearfix" id="<?php echo $row->msg_id; ?>_ID_sent_body" style="display: none;">
                            <div class="message-body-rgt-col" style="width:565px;padding-left:2px;">
                                ...
                                <div style="width: 18px;float:right;">
                                    <a href="javascript:closeSentDetail(<?php echo $row->msg_id; ?>)"><img src="images/close.png"/></a>
