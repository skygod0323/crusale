...
class listCat{
	...
	function deleterecord($adid){
		mysql_query("update notice set ntc_status = '0' where ntc_id='".$adid."'");
	}
	function deletelink($id){
		if($_SERVER['QUERY_STRING']==""){
			...
			$dellink="notice-view.php?".$_SERVER['QUERY_STRING']."&action=del&fid=".$id;
		}
		return $dellink;
	}
}
...
$al->setsql("select * from notice where ntc_status = '1' order by ntc_updated_date desc");
$totalitems=$al->totalrecord();
$limit=$al->limit;
$al->start=$p->setstart($page,$limit,$totalitems);
$adjacents=1;
$targetpage = "notice-view.php";
...
{ 
	...
	{		
		mysql_query("update notice set ntc_status = 0 where ntc_id='".$cb."'");		
	}
	header("location:notice-view.php");
}
?>
...
<?php $j=0;
	...
	if($count >0)
	{
	 	while($row=mysql_fetch_object($recObj)){
		   	?>
		<tr>
	        <td class="center"><label><input name="cb[]"  class="ace" type="checkbox" value="<?php echo $row->ntc_id; ?>" /><span class="lbl"></span></label></td>
            <td><?php echo $row->ntc_heading;	?></td>
			<td><?php echo $row->ntc_description;	?></td>
            
            
            <td style="text-align:center">
            <a href="notice-edit.php?aid=<?php echo $row->ntc_id ?>" title="Edit"><img alt="Edit" src="images/edit.jpg" border="0"></a>
            <a href="<?php echo $al->deletelink($row->ntc_id)?>" title="Delete" onclick="return confirm('Are you sure to delete the Notice?')"><img alt="Delete" src="images/suspend.jpg" border="0"></a>            
            </td>
	</tr>
        <?php $j++; } }?>
</tbody>
