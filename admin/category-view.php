...
class listCat{
	...
	function deleterecord($adid){
		...
		mysql_query("update category set cat_status = '0' where cat_id='".$adid."'");
	}
	function deletelink($id){
		if($_SERVER['QUERY_STRING']==""){
			...
			$dellink="category-view.php?".$_SERVER['QUERY_STRING']."&action=del&fid=".$id;
		}
		return $dellink;
	}
}
...
$al->setsql("select * from category where cat_status = '1'");
...
<tbody>
		...
		if($count >0)
		{
		   	...
		   	?>
    	<tr>
	        <td class="center"><label><input name="cb[]"  class="ace" type="checkbox" value="<?php echo $row->cat_id; ?>" /><span class="lbl"></span></label></td>
            <td><?php echo stripslashes($row->cat_name); ?></td>
            <td style="text-align:center"><a href="category-edit.php?fid=<?php echo $row->cat_id ?>" title="edit"><img alt="edit" src="images/edit.jpg" border="0"></a>
            <a href="<?php echo $al->deletelink($row->cat_id)?>" title="delete" onClick="return confirm('Are you sure to delete the record?')"><img alt="delete" src="images/delete.jpg" border="0"></a>
            </td>
	</tr>
        <?php $j++; } }?>
</tbody>
