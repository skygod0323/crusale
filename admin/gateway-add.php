...
class addGateway{
	...
	function __construct($pg_id, $pg_name, $pg_deposit_fee, $pg_deposit_fee_type, $pg_withdraw_fee, $pg_withdraw_fee_type, $pg_logo)
	{		
		$this->pg_id=$pg_id;
		$this->pg_name=$pg_name;
		$this->pg_deposit_fee=$pg_deposit_fee;
		$this->pg_deposit_fee_type=$pg_deposit_fee_type;
		$this->pg_withdraw_fee=$pg_withdraw_fee;
		$this->pg_withdraw_fee_type=$pg_withdraw_fee_type;
		$this->pg_logo=$pg_logo;
		
		$_SESSION['pg_id']=$this->pg_id;
		$_SESSION['pg_name']=$this->pg_name;
		$_SESSION['pg_deposit_fee']=$this->pg_deposit_fee;
		$_SESSION['pg_deposit_fee_type']=$this->pg_deposit_fee_type;
		$_SESSION['pg_withdraw_fee']=$this->pg_withdraw_fee;
		$_SESSION['pg_withdraw_fee_type']=$this->pg_withdraw_fee_type;
	}
...
//				$imgSImage->resize(119,70);
				...
				$this->msg='<div class="alert alert-success"><i class="icon-ok"></i> Payment gateway added successfully.</div>';
				
				unset($_SESSION['pg_id']);
				unset($_SESSION['pg_name']);
				unset($_SESSION['pg_deposit_fee']);
				unset($_SESSION['pg_deposit_fee_type']);
				unset($_SESSION['pg_withdraw_fee']);
				unset($_SESSION['pg_withdraw_fee_type']);
			}
		}
		else
		{
...
if(isset($_SESSION['pg_deposit_fee'])){	$pg_deposit_fee=$_SESSION['pg_deposit_fee'];	unset($_SESSION['pg_deposit_fee']); }else{ $pg_deposit_fee=""; }
if(isset($_SESSION['pg_deposit_fee_type'])){	$pg_deposit_fee_type=$_SESSION['pg_deposit_fee_type'];	unset($_SESSION['pg_deposit_fee_type']); }else{ $pg_deposit_fee_type=""; }
if(isset($_SESSION['pg_withdraw_fee'])){	$pg_withdraw_fee=$_SESSION['pg_withdraw_fee'];	unset($_SESSION['pg_withdraw_fee']); }else{ $pg_withdraw_fee=""; }
if(isset($_SESSION['pg_withdraw_fee_type'])){	$pg_withdraw_fee_type=$_SESSION['pg_withdraw_fee_type'];	unset($_SESSION['pg_withdraw_fee_type']); }else{ $pg_withdraw_fee_type=""; }

if(isset($_POST['btnAdd']))
{ 	

	$adn=new addGateway(addslashes(trim($_POST['pg_id'])), addslashes(trim($_POST['pg_name'])),addslashes(trim($_POST['pg_deposit_fee'])),addslashes(trim($_POST['pg_deposit_fee_type'])),addslashes(trim($_POST['pg_withdraw_fee'])),addslashes(trim($_POST['pg_withdraw_fee_type'])), trim($_FILES['pg_logo']['name']));
	...
	header("location:gateway-add.php");
}
...
<div class="main-content">
	<div class="breadcrumbs" id="breadcrumbs">
		...
		<ul class="breadcrumb">
			...
			<li class="active">Gateway Add</li>
		</ul><!-- .breadcrumb -->
		<!-- #nav-search -->
	</div>
...
<div class="page-content">
	...
	<div class="row">
		<div class="col-xs-12">
<form class="form-horizontal" action="" id="ar_add" name="ar_add" method="post" enctype="multipart/form-data" onsubmit="return myvalid();">
<em style="display:block;margin:5px;">Fields with <span style="color:#F00">*</span> are required.</em>
	...
	<div class="form-group">
		...
		<div class="col-sm-9">
			<input name="pg_id" id="pg_id" class="col-xs-10 col-sm-6" type="text" value="<?php echo $pg_id; ?>" />
		</div>
	</div>
     ...
     <div class="form-group">
		...
		<div class="col-sm-9">
			<input name="pg_name" id="pg_name" class="col-xs-10 col-sm-6" type="text" value="<?php echo $pg_name; ?>" />
		</div>
	</div>
    <div class="form-group">
		<label class="col-sm-3 control-label no-padding-right" for="form-field-2">Deposit Fee <span style="color:#CC0000">*</span></label>
		<div class="col-sm-9">
			<input name="pg_deposit_fee" id="pg_deposit_fee" class="col-xs-10 col-sm-6" type="text" value="<?php echo $pg_deposit_fee; ?>" />
		</div>
	</div>
    <div class="form-group">
		...
		<div class="col-sm-9">
			<select name="pg_deposit_fee_type" id="pg_deposit_fee_type">
                <option value="percent" <?php if($pg_deposit_fee_type=="percent"){ ?> selected="selected"<?php } ?>>Percent</option>
                <option value="fixed" <?php if($pg_deposit_fee_type=="fixed"){  ?> selected="selected"<?php } ?>>Fixed</option>
            </select>
		</div>
	</div>
	<div class="form-group">
		...
		<div class="col-sm-9">
			<input name="pg_withdraw_fee" id="pg_withdraw_fee" class="col-xs-10 col-sm-6" type="text" value="<?php echo $pg_withdraw_fee; ?>" />
		</div>
	</div>
    <div class="form-group">
		...
		<div class="col-sm-9">
			<select name="pg_withdraw_fee_type" id="pg_withdraw_fee_type">
                <option value="percent" <?php if($pg_withdraw_fee_type=="percent"){ ?> selected="selected"<?php } ?>>Percent</option>
                <option value="fixed" <?php if($pg_withdraw_fee_type=="fixed"){ ?> selected="selected"<?php }?>>Fixed</option>
            </select>
		</div>
	</div>
