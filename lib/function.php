...
function get_page_title($x)
{
	$sql = "select * from cms where cms_id='".$x."'";
	$qry = mysql_query($sql);
	$arr = mysql_fetch_array($qry);	
	return stripslashes($arr['cms_title']);
}
...
function getNotificationDeleteDayAfter()	//pranab
{
	$sql="select st_value from site_settings where st_id=1 and st_status='14'";
	$query=mysql_query($sql);
	if($tit=mysql_fetch_object($query))
	{
		return ucfirst($tit->st_value);
...
function profileCompleteness($uid)
{
	...
	{
		$per=getCompletenessPercentage(1);
	}
	...
	{
		$per=$per+getCompletenessPercentage(2);
	}
	...
	{
		$per=$per+getCompletenessPercentage(3);	
	}
	...
	{
		$per=$per+getCompletenessPercentage(4);	
	}
	...
	{
		$per=$per+getCompletenessPercentage(5);	
	}
	...
	{
		$per=$per+getCompletenessPercentage(6);	
	}

	return $per;
}
function getCompletenessPercentage($id)
{
	$sql="select pc_value from profile_completeness where pc_id=".$id;
	$res=mysql_query($sql);
