...
class addDispute
{
	...
	function __construct($pds_id, $pds_claimReason, $max_release_amt, $pds_claimAmount)
	{
		$this->pds_id=$pds_id;
            $this->pds_claimReason=$pds_claimReason;
            $this->max_release_amt=$max_release_amt;
		$this->pds_claimAmount=$pds_claimAmount;
	...
	{	
        ...
        if($this->pds_claimReason=="")
		{
			$this->msg='<font color="red">'.$lang[650].'</font>';
			$valid=false;
                  
		}
		else if($this->pds_claimAmount=="")
		{
			$this->msg='<font color="red">'.$lang[644].'</font>';
			$valid=false;
		}
        else if($this->pds_claimAmount > $this->max_release_amt)
        {
	...
	//	$_SESSION['pds_prj_id']=$this->pds_prj_id;
		$_SESSION['pds_claimReason']=$this->pds_claimReason;
            $_SESSION['pds_claimAmount']=$this->pds_claimAmount;            
	}
	
	function add()
	{		
		include "language.php";
		$sql="update project_dispute 
			set 
				pds_claimReason='".$this->pds_claimReason."',
				pds_claimAmount='".$this->pds_claimAmount."',
				pds_claimDate=now()";
		...
		set
			...
			un_type='dispute',
			un_content='".$_SESSION['usr'].$lang[647].getCurrencySymbol().$this->pds_releaseAmount." ".getCurrencyCode()."',
	...
	//	mysql_query($sql_un);
		...
		unset($_SESSION['pds_claimReason']);
		unset($_SESSION['pds_claimAmount']);
	}
}
if(isset($_POST['submitClaim']))
{
	$upd=new addDispute(addslashes(trim($_POST['pds_id'])),addslashes(trim($_POST['pds_claimReason'])), addslashes(trim($_POST['max_release_amt'])), addslashes(trim($_POST['pds_claimAmount'])));
	...
	//header("Location:fDispute.php?".$_SERVER['QUERY_STRING']);
}

?>

   	<?php include "includes/header.php"; ?>
    <div><h2><?php echo $disp_msg; ?></h2></div>
    
		
        <div><?php echo $lang[655]; ?></div>
